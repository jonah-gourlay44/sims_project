...
class pdd(object):
    ...
    def dNi_dNj(self, le, psi, n, n_c, p_c):
        x_j = le[1]; x_i = le[0]
        Le = (x_j - x_i) 
    ...
    def Ni_rho_eff(self, le, psi, ind, n, n_c, p_c):
        x_j = le[1]; x_i = le[0]
        Le = (x_j - x_i) 
        
        x = np.linspace(x_i, x_j, 50)
        N_i = (1 - (x - x_i)) / Le
        N_j = (x - x_i)/L_e
        psi = psi[0] + (x - x_i) * (psi[1] - psi[0]) / Le 
        exp_neg_psi = np.exp(-psi)
        exp_pos_psi = np.exp(psi)

        f = (exp_pos_psi - exp_neg_psi + (self.parameters.N_a - self.parameters.N_d)/n_i) * n_i / N
        be = np.zeros((2,1))

        be[0,0] = np.trapz(f*N_i, x)
        be[1,0] = np.trapz(f*N_j, x)
        print(be)
        return be 
